...
function generateBlacklist() {
  ...
  fs.readdirSync(packageModules).forEach(moduleDir => {
    if (moduleDir === 'react-native') {
      ...
      duplicateModules.push(new RegExp('packages' + escapedSep + '(?:(?!pirateship).*)' +
        escapedSep + 'node_modules' + escapedSep + moduleDir + escapedSep + '.*'));
      duplicateModules.push(new RegExp(rootModules.replace(/\\/g, '\\\\') + escapedSep +
        moduleDir + escapedSep + '.*'));
    } else if (moduleDir !== '.bin' && fs.existsSync(path.resolve(rootModules, moduleDir))) {
      duplicateModules.push(new RegExp('packages' + escapedSep + '.*' + escapedSep + 'node_modules' +
        escapedSep + moduleDir + escapedSep + '.*'));
    }
  });
